...
/**
 * POST /api/sekolah/pembayaran/select
 *
 * Dipakai saat no WA pembina terdaftar di >1 sekolah dan pembina memilih
 * salah satunya dari daftar di halaman "Status Pembayaran & Kwitansi".
 * Verifikasi ulang kepemilikan lalu terbitkan sesi pembayaran untuk sekolah
 * terpilih — supaya selain melihat status, pembina tetap bisa upload bukti
 * transfer bila statusnya BELUM_BAYAR/DITOLAK.
 */
export async function POST(req: NextRequest) {
  try {
    const rl = checkRateLimit(req, { key: 'status-select', max: 30, windowMs: 15 * 60 * 1000 })
    if (rl) return rl
    ...
    console.error('[POST /api/sekolah/pembayaran/select]', error)
