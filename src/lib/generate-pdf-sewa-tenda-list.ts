...
export interface SewaListExportRow {
...
export interface SewaListExportTotals {
  totalSekolah: number
  totalUnit: number
  totalBiaya: number
}

function toRows(rows: SewaListExportRow[]): string[][] {
...
export async function generatePdfSewaTendaList(
  tanggal: string,
  filterLabel: string,
  rows: SewaListExportRow[],
  totals: SewaListExportTotals,
  namaPetugas: string
): Promise<Buffer> {
  const { pdf, regular, bold } = await createRekapPdf('Sewa Tenda')
  const createPage = () => addRekapPage(pdf, 'LAPORAN SEWA TENDA', tanggal, bold, regular)
  let page = createPage()

  drawText(page, filterLabel, REKAP_A4_W / 2, 106, bold, 13, REKAP_NAVY, 'center')
